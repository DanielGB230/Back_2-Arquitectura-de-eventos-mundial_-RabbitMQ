...
public class EmailConsumer : BackgroundService
{
    ...
    private const string QueueName = "queue.email";
        ...
        ILogger<EmailConsumer> logger)
    {
        ...
        _logger.LogInformation("EMAIL-CONSUMER: Conectado a RabbitMQ.");
    }
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation("EMAIL-CONSUMER: Esperando mensajes en la cola '{QueueName}'...", QueueName);
    ...
    private async Task OnMessageReceived(object sender, BasicDeliverEventArgs ea)
    {
        ...
        _logger.LogInformation("EMAIL-CONSUMER: Mensaje recibido.");
        ...
        try
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                await emailService.HandleEventAsync(message);
            }
        ...
        {
            _logger.LogError(ex, "EMAIL-CONSUMER: Error procesando el mensaje. Enviando a NACK. Mensaje: {Message}", message);
    ...
    {
        _logger.LogInformation("Cerrando consumidor de email...");
        _channel.Close();
        _connection.Close();
